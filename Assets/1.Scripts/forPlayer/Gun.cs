...
public class Gun : MonoBehaviour
{
    ...
    private void ShotProcessOnServer()
    {
            ...
            fireTransform.forward, out hit, fireDistance))
        {
            // 레이가 어떤 물체와 충돌한 경우
            //기본적
            // 충돌한 상대방으로부터 EnemyHealth 오브젝트를 가져오기 시도
            EnemyHealth target =
                hit.collider.GetComponent<EnemyHealth>();

            // 상대방으로 부터 EnemyDamage 오브젝트를 가져오는데 성공했다면
            if (target != null)
            {
                // 상대방의 OnDamage 함수를 실행시켜서 상대방에게 데미지 주기
                target.OnDamage(damage, hit.point, hit.normal);
              
                    GameManager.instance.hitScorePlayerMaster += 10;
    ...
    private IEnumerator ShotEffect(Vector3 hitPosition)
    {
        ...
        // 총격 소리 재생
        gunAudioPlayer.PlayOneShot(shotClip,0.8f);
    ...
    private IEnumerator ReloadRoutine()
    {
        ...
        // 재장전 소리 재생
        gunAudioPlayer.PlayOneShot(reloadClip,0.8f);
